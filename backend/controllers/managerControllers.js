...
const generateReport = (req, res) => {
  ...
  SELECT
      ...
      (
          SELECT GROUP_CONCAT(CONCAT(destination_city, ': ', reservations, ' reservations') SEPARATOR '\n')
          FROM (
              SELECT
                  f.destination_city,
              ...
              ORDER BY
                  COUNT(*) DESC
              LIMIT 5
          ) AS popular_destinations_subquery
      ) AS popular_destinations,
      (SELECT AVG(ticket_price) FROM Flights) AS average_booking_value
  FROM
      Reservations r
  JOIN
      Flights f ON r.flight_id = f.flight_id
...
connection.query(query, [startDate, endDate, startDate, endDate], (err, results) => {
